...
+ (void)initialize
{
	if (self != [MQBlurView class]) {
		return;
	}

	MQBlurView *appearance = [self appearance];
...
- (void)blurSnapshot
{
    ...
    dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void){
        ...
        //Background Thread
        UIImage *image = [self.image applyLightEffect];
